...
完成本計畫後，開發者可以啟動本地 MariaDB 10.11 與 Next.js，使用 server 提供的 Local Identity 建立一份 HUB_MANAGED 文件、讀取內容，並透過 org → source → folder/document tree 找到同一份文件。Domain、repository 與 transaction 測試證明 stable ID、immutable revision、ownership、archive／restore 與 source version guard 的規則。
...
- SourceEntry、KnowledgeAsset、SyncRun 建 schema 與必要 repositories；使用 fixtures 驗證 mapping 與 transaction，不建 Folder scanner、parser、diff engine 或 Sync UI。
...
| IDs | server 以 UUID 產生，資料庫使用一致的 `CHAR(36)` ASCII binary collation | 不使用路徑、title 或 content hash 產生 Document identity |
...
| T03 | 八張核心表與 referential／unique／CHECK constraints | §5、§5.1 |
| T04 | Domain models、純規則、ports 與 unit tests | §4、§6、§8.1 |
| T05 | SQL repositories、same-connection transaction 與 rollback tests | §6.4、§7.1 |
...
全部核心表使用 InnoDB。跨表 ID 型別／長度／collation 一致；文字內容使用 utf8mb4；可見文字與 opaque ID 不共用大小寫不敏感的比較語意。時間統一 UTC、`DATETIME(6)`；revision Markdown 用 LONGTEXT；knowledge metadata 用 JSON。
...
| 操作者 reference | Source／Document／Revision 的 created_by 與 SyncRun.triggered_by 指向 users.id；使用者姓名／org 更新不改 reference |
...
MariaDB infrastructure 在一次 `run` 中取得一條 connection，begin → callback → assertions → commit；失敗 rollback；finally release。所有 transaction-bound repository 都綁同一 connection，不能改用 `pool.query()` 跳出交易。官方 connector 提供相應 connection transaction methods。[MariaDB Node.js Promise API](https://mariadb.com/docs/connectors/mariadb-connector-nodejs/connector-nodejs-promise-api)

Knowledge application operations 分兩層：面向一般 caller 的 service wrapper 取得 identity／開啟 UoW；內部 mutation functions 接受已綁定的 repositories，不自行 commit。後續 Sources orchestration 可在自己的 UoW 呼叫相同 mutation functions。不能使用 UI 傳入的 `isSync`／ownership flag 繞過唯讀限制；受控來源操作不 export 成 Web Server Action。
...
影響列數必須為 1，否則整個 callback 不得進行 Knowledge writes，回報找不到來源／不可同步／版本已變的對應錯誤。Version increment、Knowledge writes、mapping 與 APPLIED record 都在同一交易；後續失敗時版本也 rollback。需要來源 lock 的內部 operation 可使用此交易已取得的 lock。
...
3. 選定 Node LTS 與相容依賴，鎖定實際版本；加入官方 mariadb driver、Vitest、Playwright、tsx 與必要 lint tooling。
...
**依賴：** T02。**交付：** design spec 八張核心表、§4 的 SQL 保護與 schema tests。
...
tests/unit/{content,ownership,lifecycle,tree,source-mapping}.test.ts
```

工作：

1. UserIdentity 精確四個 string 欄位；source type 與 ownership 透過 domain validation 限制合法組合。
...
3. 比較 canonical representation 決定是否變更，再產生內容 hash；metadata 與 title 不能被漏掉。UUID generator 與 clock 可注入供測試，不影響 domain identity 定義。
4. 定義同文件更新、rename／move、archive／restore 與已知 mapping reappearance 的規則；不知道 external identity 時不猜同一文件。
...
6. 定義 §4.4 的 UoW 與 ports；不建立 database-neutral 巨型 repository framework。

驗證：unit tests 涵蓋 title-only、metadata-only、body-only、identical input、metadata key-order、path-only、restore-same-content、restore-changed-content；SOURCE_MANAGED 的 Hub mutations 被拒絕。Vitest 只測 domain/application 行為，Next server components 的真正互動留給 Playwright。[Vitest guide](https://vitest.dev/guide/)

### T05 — Repositories 與同連線 transaction

**依賴：** T03、T04。**交付：** 真實 DB 的資料存取、atomicity 與錯誤轉換。
...
1. 所有 writes 使用 parameterized SQL；在 repository 邊界轉換 JSON／UTC time／nullable values，驗證 counter 為合法 integer。
2. 實作一條 connection 綁定一組 repositories 的 UoW；所有 callback promise 必須 await，callback 失敗 rollback，connection 用完 release。
...
5. 建立流程的 pre-commit assertions 驗證本次建立文件有合法 current revision。Repository 的 revision API 不提供 overwrite／delete。

驗證：真實 DB 逐點注入失敗（Document、Revision、pointer、Tree 完成後、commit 前）；每次用另一條連線確認無孤兒資料，涵蓋各 repository 已寫入後才失敗的情境。成功時四者一致可讀；另測 connection 釋放後下一次操作可正常使用。

### T06 — Local Identity 與可重複使用的 fixtures

**依賴：** T05。**交付：** 固定四欄位身分、最小本地資料與非 Web 測試入口。
...
2. `users` 以穩定 id／unique emp_id 對齊，更新 name／org 不重建 id；發現 id 與 emp_id 分別對到不同使用者時拒絕，不合併兩筆身分。未來 SSO provider 可替換，不預建 SSO SDK 或 token parser。
3. 用明確 `KM_LOCAL_IDENTITY_ENABLED` 配置啟用本機示範與 E2E；`NODE_ENV=production` 的本地 build smoke 可明確啟用 local mode，但文件清楚標示其非企業登入。Server composition 在 local provider 以外尚無可用 adapter 時回報未配置，不默默降級。
4. Development seed 建一個本地使用者、一個 HUB_MANAGED Source、一個 Folder；seed 以固定 IDs 確保重跑不複製資料，且不覆蓋使用者建立的文件。
5. Integration fixtures 另準備兩個 org、兩個 Hub Source、一個 SOURCE_MANAGED Source、已識別 SourceEntry、ACTIVE／ARCHIVED 文件；不放入正式 DB，不使用真實公司身分。
6. `src/server/composition.ts` 注入 identity、UoW 與 repositories；只有 server adapters import 此檔，domain 不 import composition。

驗證：provider 四欄位、缺少設定失敗、工號碰撞拒絕、使用者更新保留 id；seed 執行兩次不增加重複 Source／Folder。偽造前端 emp_id／org_code 不影響實際 caller。這不測試或宣稱完整 ACL／跨組織分享已完成。
...
1. `createHubManagedDocument`：解析基本輸入、取得可信 caller、鎖定與驗證有效 Hub-managed Source／parent，執行 §4.3 原子建立流程，回傳 stable Document ID。
2. `getDocument`／`getCurrentRevision`／`listTree`：讀同文件 current revision；Tree DOCUMENT 名稱取 current title，Folder 取 name；預設不列 archived Source／node／Document，亦不列 archived 祖先底下節點。預設 document read 不將 archived 當有效文件回傳；明確的歷史讀取模式供內部測試驗證資料仍保留。
3. `createRevision`：鎖內比較 title／Markdown／metadata；有變更才 insert next revision + update pointer；相同輸入回傳既有 revision。舊內容與作者／建立時間不變。
4. `archiveDocument`／`restoreDocument`：Hub-managed 的一般操作與受控來源內部操作分開；Knowledge 更新 Document 與其關聯 Tree，保留全部歷史。有 SourceEntry 的來源操作由 Sources caller 在同一 UoW 更新 mapping 狀態，Knowledge 本身不寫 SourceEntry。Restore 需有有效 parent；不暗中 reparent 或 restore 整棵 archived folder。Folder subtree 的完整管理 UX 交由 Phase 1／2。
5. 最小 `moveTreeNode`／`renameFolder`／`reorderNode`：同 Source 內依 §4.5 驗證，SOURCE_MANAGED 的一般 Hub 操作拒絕；操作不改 Document ID 或 revision。Document title 修改仍走 createRevision，沒有另存 Tree document name。
6. 受控來源使用的內部 Knowledge mutation 接受 Document ID、正規化內容與可信 execution scope；由 Sources caller／fixture 先解析已知 SourceEntry mapping，再呼叫它。可 archive／restore／改內容並沿用 Document ID，Knowledge 不 import SourceEntry 型別或 matching 實作。
7. 每個 public command 都要從 server identity／可信執行 context 取得 caller，不由 UI 指定 ownership 或 mutation mode。核心 command 不 import Next.js，Web revalidation／redirect 留在 adapter。

驗證：透過 application service 而不是 page 呼叫建立、相同內容重送、title-only 更新、metadata-only 更新、move／rename、archive／restore。讀出 DB 證明 ID 穩定、舊 revision 未變、current pointer 正確；SOURCE_MANAGED 所有 Hub mutation 都被拒絕且無寫入。新增兩個 concurrent move 的 cycle case 與同文件 simultaneous revision allocation case。

完成界線：不建立全文搜尋、完整歷史瀏覽 UI、權限角色管理、完整 authoring 或檔案 upload UI。
...
1. Sources application guard 經 source-repository port 呼叫 §4.6 的 condition-update；SQL 放在 MariaDB repository。實作 Sources transaction port，讓 source repositories 與 Knowledge mutations 共用 connection。
2. 使用已解析且已知 mapping 的 fixture，在交易中執行 Document／Revision／Tree 更新、SourceEntry／asset metadata 寫入、source version 遞增與 APPLIED run 保存。不新增正式 `syncFolder()` engine 或 Preview／Confirm route。
...
5. 選擇 Knowledge write、entry update、asset metadata write 與 APPLIED record 前等失敗點，驗證所有 canonical changes／版本／成功紀錄都回滾。
...
驗證：上述 integration tests 使用 MariaDB 10.11 真實 commits、獨立連線與有期限的 barrier。任一 suite 缺 DB 不可 silent skip 後算驗收通過。測試不依賴實際 folder、ZIP、binary files 或外部 API。
...
2. Form 只收 source／parent／title／Markdown 等內容欄位；server action 再解析、驗證與呼叫 application service，不能把 UI 顯示的 readonly 或 ownership 當安全檢查。
3. Create 成功後由 adapter 更新畫面／導航到 stable document URL；重新整理仍讀到同一文件與 current revision。Identity／DB read 在 request 階段執行，不在 build 靜態擷取本地資料，也不共享快取 caller-specific data。
...
1. 寫清楚 runtime／dependency lock、Docker、env、migration、seed、啟動、測試與 test DB 清理方式；記錄實際 MariaDB 10.11 patch／image。
2. 按 §6 執行驗收命令；每次 failed check 附修復與重跑結果，不以完成文件或 build 成功代替 DB concurrency／E2E 證據。
3. 核對 modules 的 import direction、transaction connection 共用、SOURCE_MANAGED guards、current pointer 與 archive filtering，確認 non-Web integration caller 直接使用 application services。
4. 核對 manifest／資料表／路由沒有引入排除依賴或 Folder Sync、SSO、publishing、search/vector、MCP、memory、binary storage、hard delete 功能。
5. Verification 記錄各命令、執行日期、版本、通過／失敗、剩餘限制。若 DB 或 browser 環境無法跑，列明未驗證項目並保持 Phase 0 未完成。
6. 交接 Phase 1／2：現有 ports、transaction 使用方式、source policy 接點、已知 mapping 的用法與後續功能邊界。不要提前產生 Phase 1 實作計畫或部署。
...
| `test:unit` | Vitest run，包含 ownership／content／lifecycle／Tree；不連 DB |
...
| ID | 情境 | 必須觀察到的結果 | 主要任務 |
| --- | --- | --- | --- |
| A01 | 新 test DB migration，重跑 migration | 核心八表＋工具 ledger 建立；重跑無重複 schema | T02–T03 |
...
| A06 | filename／Folder rename、move、reorder | stable Document ID 與 revision 不變 | T04、T07、T08 |
| A07 | Hub 試圖修改 SOURCE_MANAGED | service 拒絕且 DB 無變更；不能靠 UI flag 繞過 | T07 |
| A08 | Archive 後讀 Tree，再 restore | 預設 Tree 不列 archived；history／ID 保留，restore 後正常出現 | T07 |
...
| A12 | 兩條 connection 同 base version 競爭 | 最多一次成功；loser 無 Knowledge／version writes | T08 |
| A13 | Source Apply fixture 寫一半失敗 | Knowledge／Tree／Entry／asset／version／APPLIED 全 rollback | T08 |
| A14 | 主交易失敗後保存 FAILED | FAILED 獨立存活；沒有 result_version；Knowledge 仍回滾 | T08 |
| A15 | All-unchanged 成功 Apply fixture | 只增加 source version／run，revision／Tree／archive 不變 | T08 |
| A16 | 對 archived 或不同 Source parent 操作 | 拒絕無效結構；不偷偷 reparent／改 Source | T07 |
| A17 | concurrent moves 可能合成 cycle | Source lock 與 ancestry validation 阻止非法結果 | T07 |
| A18 | Local identity + 偽造前端 emp_id／org | server 仍回傳設定的四欄位可信測試身分 | T06 |
| A19 | 頁面建立後重新整理／Tree 導航 | 同一 stable URL／Document ID／current revision | T09 |
| A20 | 直接 non-Web 呼叫 application | 不需 React／Next runtime 就可完成 core/query integration test | T07、T10 |
| A21 | Asset metadata 寫入與讀取 | 保存 path／metadata，沒有 binary storage／serving | T03、T05 |

## 8. 完成檢核與後續責任

以下 checkbox 在程式實作完成並有證據後才勾選；本計畫交付時全部維持未完成。

- [ ] T01：技術骨架、版本鎖定、lint/typecheck/build 可用。
- [ ] T02：Local MariaDB 10.11、migration runner、隔離測試 DB 可用。
- [ ] T03：八張核心表、複合 FK／CHECK／UNIQUE 經真實 DB 驗證。
- [ ] T04：Domain models／ports 與核心規則 unit tests 通過。
- [ ] T05：Repositories 與共用 connection transaction／rollback 通過。
- [ ] T06：Local identity 四欄位與 seed 可用，無前端身分信任捷徑。
...
| --- | --- | --- |
| Phase 1 | Knowledge models、repositories、commands／queries、Tree invariants | 完整 Knowledge／Tree 產品能力與管理 UI |
| Phase 2 | SourceEntry／asset／SyncRun repositories、version guard、Sources UoW、內部 Knowledge mutations | Folder 非 ZIP 上傳、掃描／解析、缺 ID matching／歧義、diff、Preview storage／期限／snapshot binding、首次 Source 建立與完整 Confirm／Apply 流程 |
| Phase 3 | 四欄位 IdentityProvider、Source org owner、caller context | 真正授權政策／分享／公司 SSO；不能把 Phase 0 local fixture visibility 當企業 ACL |
| Phase 4–9 | UI-independent application core、stable IDs／revisions、清楚 consumer boundary | Discovery、authoring、publishing、MCP、semantic retrieval、memory／relations |

Phase 0 的內部源更新 fixtures 不保證任意 folder rename 可被識別；Phase 2 必須在既有契約下定義匹配與 Preview 的產品規則。Phase 0 的 revision row locks 不代表完整作者衝突處理已完成。資產 metadata 不代表附件可存取。

## 9. 本計畫自我審查

| 面向 | 審查結論 |
| --- | --- |
| 對齊 design spec | 每個 task 對照已確認章節；八張核心表、最小 identity、ownership、revision 與交易規則均有工作及驗證 |
...
| SourceEntry ambiguity | 不以 path/hash 唯一鍵假裝解決來源辨識；只對已識別 identity 建約束與 fixtures |
| Sync scope | 僅 version guard、repositories 與 atomicity fixture；沒有建 scanner／Preview／Confirm／Apply 產品入口 |
| NOOP／FAILED | NOOP 不增 revision；成功版本可前進；FAILED 另交易保存 |
| Identity／governance | 四欄位不擴張；mock 由 server 配置；同 org 不自動成為企業授權規則 |
...
| 範圍控制 | 沒有預裝後續 editor／drag／search／MCP／publishing／memory 或 binary storage |
