  ...
  return <>
    <details ref={menu} className="group relative min-w-0 w-full" onKeyDown={(event) => {
      ...
      <summary aria-label={`Workspace: ${currentName}`} title={`Switch workspace: ${currentName}`}
        className="flex min-h-10 w-full cursor-pointer list-none items-center gap-2 rounded-md border border-kh-border px-3 text-sm hover:bg-kh-bg-hover focus-visible:ring-2 focus-visible:ring-kh-accent [&::-webkit-details-marker]:hidden">
