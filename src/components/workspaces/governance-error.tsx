...
export async function governanceRequest<T>(url: string, method = "GET", body?: unknown): Promise<T> {
...
export function governanceFailure(error: unknown): GovernanceFailure {
  return error instanceof GovernanceRequestError ? error.failure : { code: "REQUEST_FAILED", message: "Unable to connect. Please try again." };
}
export function GovernanceError({ error }: { error: GovernanceFailure | null }) {
  if (!error) return null;
  const message = error.code === "NOT_FOUND" ? "This resource is not available." : error.code === "WORKSPACE_ARCHIVED" ? "This workspace is archived and read-only. Refresh to see its current state." : error.code === "INSUFFICIENT_WORKSPACE_CAPABILITY" || error.code === "TEAM_CREATION_DENIED" ? "You no longer have permission to perform this action." : error.message;
