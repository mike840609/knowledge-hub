...
function folderOptions(items: TreeItem[], prefix: string[] = []): FolderOption[] {
  return items.flatMap((item) => item.nodeType === "FOLDER"
    ? [{ id: item.id, label: [...prefix, item.name].join(" / ") }, ...folderOptions(item.children, [...prefix, item.name])]
    : []);
}
...
export default async function KnowledgePage({ searchParams }: { searchParams?: Promise<{ workspaceId?: string }> }) {
  ...
  const sources = selectedWorkspaceId ? await services.sources.listSources(caller, selectedWorkspaceId) : [];
  const sourceTrees = await Promise.all(sources.map(async (source) => ({ source, tree: await services.knowledge.listTree(caller, source.id) })));
  return (
